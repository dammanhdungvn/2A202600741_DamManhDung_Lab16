    ...
    def run(self, example: QAExample) -> RunRecord:
        ...
        for attempt_id in range(1, self.max_attempts + 1):
            ...
            if self.agent_type == "reflexion" and attempt_id < self.max_attempts:
                reflection, ref_tokens, ref_latency = reflector(example, attempt_id, judge, answer)
                trace.reflection = reflection
                trace.token_estimate += ref_tokens
                trace.latency_ms += ref_latency
                
                reflections.append(reflection)
                reflection_memory.append(f"Attempt {attempt_id} Failed. Lesson: {reflection.lesson}. Strategy for next attempt: {reflection.strategy}")
